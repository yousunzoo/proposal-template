...
const cardCls =
  'relative overflow-hidden rounded-[10px] border border-line bg-surface transition-colors duration-200 hover:border-line-strong';

/** 표시된 섹션에 데이터가 없을 때의 조용한 안내 (빈 껍데기 방지) */
function EmptyNote({ children }: { children: React.ReactNode }) {
  return (
    <p className="rounded-[10px] border border-dashed border-line bg-elevated px-5 py-8 text-center text-[14px] text-ink-500">
      {children}
    </p>
  );
}
          ...
          <dl className="mt-10 flex flex-wrap gap-x-10 gap-y-4 border-t border-line pt-6">
            {meta.map((m) => (
              <div key={m.k} className="flex flex-col gap-1">
                <dt className="text-[12px] font-medium text-ink-500">{m.k}</dt>
                <dd className="text-[15px] font-semibold text-ink-900">{m.v}</dd>
              </div>
            ))}
          </dl>
        )}
          ...
          {about.values.map((v, i) => (
            <Reveal key={i} delay={i * 0.06}>
              <div className={cn(cardCls, 'flex h-full flex-col gap-3 p-6 lg:p-7')}>
                ...
                <p className="text-[14px] leading-[1.7] text-ink-600 lg:text-[15px]">{v.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
...
/* ══════════════ 3. 팀 소개 ══════════════ */
export function TeamSection({ team, index }: { team: Doc['team']; index: string }) {
  return (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {team.groups.map((g, i) => (
            <Reveal key={i} delay={(i % 4) * 0.05}>
              <div className={cn(cardCls, 'flex h-full flex-col p-6')}>
                <span className="inline-flex w-fit rounded-md border border-line bg-elevated px-2.5 py-1 text-[12px] font-semibold tracking-[-0.01em] text-ink-700">
                  {g.dept}
                </span>
                <ul className="mt-5 flex flex-col gap-3.5">
                  {g.members.map((m, mi) => (
                    <li key={mi}>
                      <p className="text-[17px] font-bold tracking-[-0.3px] text-ink-900">
                        {m.name}
                        {m.en && <span className="sr-only"> ({m.en})</span>}
                      </p>
                      {m.en && (
                        <p
                          aria-hidden="true"
                          className="text-[11px] font-medium uppercase tracking-wide text-ink-500"
                        >
                          {m.en}
                        </p>
                      )}
      ...
      {team.bullets.length > 0 && (
        <div className="mt-8 grid gap-2.5">
          {team.bullets.map((b, i) => (
            <Reveal key={i}>
              <p className="flex items-start gap-2.5 text-[14px] leading-[1.6] text-ink-700 lg:text-[15px]">
          ...
          {strategy.items.map((s, i) => (
            <Reveal key={i} delay={(i % 2) * 0.05}>
              <div className={cn(cardCls, 'flex h-full flex-col p-6 lg:p-7')}>
                <span className="text-[15px] font-semibold tabular-nums text-blue-600">{s.index}</span>
                <h3 className="mt-2 text-lg font-bold tracking-[-0.4px] text-ink-900 lg:text-xl">
                  {s.title}
                </h3>
                <p className="mt-3 text-[15px] leading-[1.7] text-ink-600">{s.body}</p>
                {s.tag && (
                  <div className="mt-6">
                    <Chip>{s.tag}</Chip>
                  </div>
        ...
        {metrics.map((m, i) => (
          <Reveal key={m.label} delay={i * 0.05}>
            <div className={cn(cardCls, 'p-7')}>
              <p className="text-[13px] font-medium text-ink-500">{m.label}</p>
              <p className="mt-3 text-[28px] font-bold tracking-[-0.8px] text-ink-900 lg:text-[34px]">
                {m.value}
              </p>
            </div>
          </Reveal>
        ...
        <div className="mb-12 flex max-w-[820px] flex-col gap-5">
          {descParas.map((p, i) => {
            ...
            if (head) {
              return (
                <div key={i}>
                  <h3 className="text-[16px] font-bold tracking-[-0.01em] text-ink-900">
                    {head[1].trim()}
                  </h3>
                  {head[2].trim() && (
                    <p className="mt-2 text-[15px] leading-[1.8] text-ink-600">
                      {head[2].trim().replace(/\n+/g, ' ')}
                    </p>
                  )}
                </div>
            ...
            return (
              <p key={i} className="text-[15px] leading-[1.8] text-ink-600">
                {p.replace(/\n+/g, ' ')}
              </p>
            );
          })}
          ...
          {items.map(({ slug, thumbnail, title, category }, i) => (
            <li key={slug}>
              <Reveal delay={(i % 3) * 0.05}>
                <Link
                  href={`${itemBase}/${slug}`}
                  className="group flex h-full flex-col rounded-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-canvas"
                >
                  <div className="relative aspect-[4/3] overflow-hidden rounded-[10px] border border-line">
                    <Image
                      src={thumbnail}
                      alt={`${title} 썸네일`}
                      fill
          ...
          {architecture.layers.map((l, i) => (
            <Reveal key={i} delay={(i % 4) * 0.04}>
              <div className={cn(cardCls, 'flex h-full flex-col gap-3 p-6')}>
                ...
                <p className="text-[14px] leading-[1.65] text-ink-600">{l.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
          ...
          <table className="w-full min-w-[560px] border-collapse text-left">
            <thead>
              <tr className="border-b border-line bg-elevated">
                <th scope="col" className="w-[180px] px-6 py-3.5 text-[13px] font-semibold text-ink-500">영역</th>
                <th scope="col" className="px-6 py-3.5 text-[13px] font-semibold text-ink-500">구현 내용</th>
                <th scope="col" className="px-6 py-3.5 text-[13px] font-semibold text-ink-500">완료 기준</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-line">
              {qa.items.map((q, i) => (
                <tr key={i}>
                  <th scope="row" className="px-6 py-4 align-top text-[14px] font-semibold text-blue-600">
                    {q.area}
                  </th>
                  <td className="px-6 py-4 align-top text-[14px] leading-[1.6] text-ink-700">{q.work}</td>
                  <td className="px-6 py-4 align-top text-[14px] leading-[1.6] text-ink-600">
    ...
    <SectionShell id="timeline" index={index} label="진행 일정" title={timeline.title}>
      {timeline.phases.length === 0 ? (
        ...
        <>
            ...
            <div className="min-w-[640px]">
              ...
              >
                {Array.from({ length: weeks }, (_, i) => (
                  <div key={i} className="pb-2 text-center text-[12px] font-medium text-ink-500">
                    {i + 1}주
                  </div>
                ))}
              </div>
                ...
                {timeline.phases.map((p, i) => (
                  <Reveal key={i}>
                    <div className="grid items-center" style={{ gridTemplateColumns: '180px 1fr' }}>
                      <span className="pr-4 text-[14px] font-semibold text-ink-700">{p.label}</span>
                      <div
                        className="grid gap-1 rounded-lg bg-elevated p-1"
                        style={{ gridTemplateColumns: `repeat(${weeks}, minmax(0, 1fr))` }}
                      >
                        <div
                          className="flex h-9 items-center justify-center rounded-md bg-blue-600 text-[12px] font-semibold text-white"
                          style={{
          ...
          <Reveal delay={0.1}>
            <p className="mt-8 rounded-xl border border-line bg-elevated px-5 py-4 text-[14px] leading-[1.7] text-ink-600">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Reveal>
          <div className={cn(cardCls, 'grid h-full gap-6 p-6 sm:grid-cols-2')}>
            <div>
              <p className="mb-3 text-[13px] font-semibold text-green-600">포함 항목</p>
            ...
            <div>
              <p className="mb-3 text-[13px] font-semibold text-ink-500">미포함 (별도 협의)</p>
              <ul className="flex flex-col gap-2.5">
                {warranty.excluded.map((w, i) => (
                  <li key={i} className="flex items-start gap-2 text-[14px] text-ink-500">
        ...
        <Reveal delay={0.05}>
          <div className={cn(cardCls, 'flex h-full flex-col gap-4 p-6')}>
            <p className="text-[13px] font-semibold text-blue-600">인수인계 항목</p>
            {warranty.handoff.map((h, i) => (
              <div key={i}>
                <p className="text-[15px] font-semibold text-ink-900">{h.title}</p>
                <p className="mt-1 text-[14px] leading-[1.65] text-ink-600">{h.body}</p>
              </div>
            ))}
          </div>
        </Reveal>
            ...
            {promise.commitments.map((c, i) => (
              <Reveal key={i}>
                <div className="flex gap-3">
                  ...
                  <p className="text-[15px] leading-[1.7] text-ink-700">{c}</p>
                </div>
              </Reveal>
            ))}
          </div>
